				...
				<table class="guests-page__table">
					<thead>
						<tr class="guests-page__row guests-page__row--head">
							<th class="guests-page__cell guests-page__cell--head">
								ФИО гостей
							</th>
							<th class="guests-page__cell guests-page__cell--head">
								Кол-во гостей
					...
					<tbody>
						<template v-for="(group, groupIdx) in getGroups()" :key="groupIdx">
							<tr v-for="(guest, idx) in group.guests" :key="guest.id + '-' + idx" :class="[
								'guests-page__row',
								guest.isMain
								...
								<td :class="[
									...
									<div class="guests-page__cell-content guests-page__cell-content--minimal">
								...
								<td v-if="idx === 0" :rowspan="group.guests.length"
				...
				<div class="guests-page__modal-minimal">
					<div class="guests-page__modal-icon">
						<svg width="38" height="38" viewBox="0 0 38 38" fill="none">
							...
							<path d="M19 11V21" stroke="#d32f2f" stroke-width="2.2" stroke-linecap="round" />
							<circle cx="19" cy="27" r="1.5" fill="#d32f2f" />
						</svg>
					</div>
					<div class="guests-page__modal-text guests-page__modal-text--minimal">
						<span v-if="deleteTarget.type === 'group'">Удалить всю группу <b>«{{ deleteTarget.name }}»</b> и
							всех её
							гостей?</span><span v-else>Удалить гостя <b>«{{ deleteTarget.name }}»</b>?</span>
					</div>
					<div class="guests-page__modal-actions guests-page__modal-actions--minimal">
						<button class="guests-page__modal-btn guests-page__modal-btn--danger" @click="confirmDelete"
							:disabled="deleting">
							{{ deleting ? "Удаление..." : "Удалить" }}
						</button>
						<button class="guests-page__modal-btn guests-page__modal-btn--ghost" @click="cancelDelete"
							:disabled="deleting">
							Отмена
						</button>
					</div>
